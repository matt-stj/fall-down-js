...
function ballCollideswithGates(ball, gates, i) {
  if ( ballOnTopofGate(ball, gates, i)
       && ball.x > GatesEdges.rightGate(gates, i)
       || ballOnTopofGate(ball, gates, i)
       && ball.x < GatesEdges.leftGate(gates, i))
    {
...
function ballFallsBetweenGates(ball, gate, gates, i, game){
  if (GatesEdges.bottomOfGate(gates, i) < BallEdges.topOfBall(ball)
  && gate.scoreable === true){
